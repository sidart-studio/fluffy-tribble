...
export function buildCityGround(scene) {
  ...
  const lots = new Map(), cells = new Map();
  ...
  return { lots, cells, slab, rim, span, highlight: hl, select: sel };
}
...
export class City {
  ...
  /** The slab of earth is exactly the land you own; nothing exists beyond it. */
  resizeLand() {
    const k = ((this.rings * 2 + 1) * PITCH) / this.ground.span;
    this.ground.slab.scale.set(k, 1, k);
    this.ground.rim.scale.set(k, 1, k);
  }
