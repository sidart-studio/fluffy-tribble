...
// ---------------------------------------------------------------- ground
/**
 * One slab of land exactly the size of the city, nothing beyond its edge.
 * Per block a "cell" (pavement + the half streets around it) that stays bare
 * earth until its ring of land is bought.
 */
export function buildCityGround(scene) {
  const span = (RADIUS * 2 + 1) * PITCH;
  const slab = new THREE.Mesh(new THREE.BoxGeometry(span, 2.4, span), cmat(0x8a6a4a, { roughness: 1 }));
  slab.position.y = -1.26; slab.receiveShadow = true; scene.add(slab);
  const rim = new THREE.Mesh(new THREE.BoxGeometry(span + 0.4, 0.3, span + 0.4), cmat(0x5d4433, { roughness: 1 }));
  rim.position.y = -0.21; scene.add(rim);
  const plotMat = cmat(0xb59a72, { roughness: 1 });
  ...
  const lots = new Map(), cells = new Map(), plots = new Map();
  ...
  for (let i = -RADIUS; i <= RADIUS; i++) for (let j = -RADIUS; j <= RADIUS; j++) {
    ...
    lots.set(`${i},${j}`, lot);
    // land you don't own yet: a bare plot outline on the earth
    if (ring > 1) {
      const plot = new THREE.Mesh(new THREE.PlaneGeometry(BLOCK + 2, BLOCK + 2), plotMat);
      plot.rotation.x = -Math.PI / 2; plot.position.set(cx, -0.04, cz); plot.receiveShadow = true; scene.add(plot); plots.set(`${i},${j}`, plot);
    }
  }
  ...
  return { lots, cells, plots, highlight: hl, select: sel };
}
...
export class City {
  ...
  buyRing() {
    ...
    for (const [k, cell] of this.ground.cells) {
      ...
      if (this.ring(i, j) === this.rings) {
        ...
        const plot = this.ground.plots.get(k); if (plot) plot.visible = false;
      }
    }
    this.cache = null;
    return true;
  ...
  clear() {
    ...
    while (this.rings > 1) {
      for (const [k, cell] of this.ground.cells) {
        const [i, j] = k.split(',').map(Number);
        if (this.ring(i, j) === this.rings) { cell.visible = false; const lot = this.ground.lots.get(k); if (lot) lot.visible = false; const plot = this.ground.plots.get(k); if (plot) plot.visible = true; }
      }
      this.rings -= 1;
    }
